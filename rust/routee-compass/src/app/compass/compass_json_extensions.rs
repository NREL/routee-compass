...
impl CompassJsonExtensions for serde_json::Value {
    ...
    ///   1. top-level of JSON is an array -> it directly
    ///   2. top-level of JSON is object without a "queries" field -> wrap it in an array and it
    ///   3. top-level of JSON is object with a "queries" field ->  if the value at "queries" is an array, it
    fn get_queries(&self) -> Result<Vec<serde_json::Value>, CompassAppError> {
        match self {
