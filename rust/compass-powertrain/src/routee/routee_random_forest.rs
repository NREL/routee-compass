...
use compass_core::model::{
    ...
    traversal::{
        function::{
            cost_function_error::CostFunctionError,
            function::EdgeCostFunction
        },
        state::{search_state::StateVector, state_variable::StateVar}, traversal_error::TraversalError,
    },
};
...
pub fn build_routee_random_forest(routee_model_path: String, 
    ...
    // Build speed table
    let speed_table = build_edge_velocity_lookup(speed_table_file)?;
    // Load random forest binary file
    let rf_binary = std::fs::read(routee_model_path.clone()).map_err(|e| CostFunctionError::FileReadError(routee_model_path.clone(), e.to_string()))?;
    let rf: RandomForestRegressor<f64, f64, DenseMatrix<f64>, Vec<f64>> =
        bincode::deserialize(&rf_binary).map_err(|e| CostFunctionError::FileReadError(routee_model_path.clone(), e.to_string()))?;
    ...
    let f: EdgeCostFunction = Box::new(move |o, e, d, s| {
        ...
        //
        let (speed_kph, state) = speed_table(o, e, d, s)?;
...
mod tests {
    ...
    fn test_edge_cost_lookup_from_file() {
        ...
        let rf_predictor = build_routee_random_forest(String::from(model_file_name), String::from(speed_file_name)).unwrap();
