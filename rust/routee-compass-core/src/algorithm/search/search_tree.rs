...
impl SearchTree {
    ...
    pub fn get_parent(&self, label: &Label) -> Option<&SearchTreeNode> {
        self.get(label)?.parent.as_ref().and_then(|parent_label| self.get(parent_label))
    }
    ...
    pub fn reconstruct_path(&self, target_label: &Label) -> Result<Vec<EdgeTraversal>, SearchTreeError> {
        let mut path = Vec::new();
        let mut current_label = target_label;

        // Walk up from target to root
        loop {
            let current_node = self.get(current_label)
            ...
            // Move to parent
            current_label = current_node.parent.as_ref()
    ...
    pub fn backtrack(&self, leaf_vertex: VertexId) -> Result<Vec<EdgeTraversal>, SearchTreeError> {
        ...
        let target_label = self.find_label_for_vertex(leaf_vertex)
    ...
    pub fn backtrack_with_direction(&self, leaf_vertex: VertexId, direction: Direction) -> Result<Vec<EdgeTraversal>, SearchTreeError> {
        let target_label = self.find_label_for_vertex(leaf_vertex)
            .ok_or_else(|| SearchTreeError::VertexNotFound(leaf_vertex))?;
    ...
    fn find_label_for_vertex<'a>(&'a self, vertex: VertexId) -> Option<&'a Label> {
        self.nodes
            .keys()
            .find(|label| label.vertex_id() == vertex)
    }
    ...
    pub fn from_search_tree_branches(
        ...
        if roots.len() != 1 {
            return Err(SearchTreeError::InvalidBranchStructure(
                format!("Expected exactly one root, found {}. Potential roots: {:?}, Child labels: {:?}", 
                    roots.len(), 
                    potential_roots, 
                    child_labels)
            ));
        ...
        while !pending.is_empty() {
            ...
            if !progress {
                return Err(SearchTreeError::InvalidBranchStructure(
                    format!("Circular dependencies or missing parents in branch structure. Remaining: {:?}", 
...
mod tests {
    use super::*;
    use crate::model::{
        network::{VertexId, EdgeId, EdgeListId}, 
        unit::Cost,
    };
    ...
    fn test_insert_child_nodes() {
        ...
        tree.insert(child1_label.clone(), child1_traversal.clone(), root_label.clone()).unwrap();
        
        // Insert second child
        let child2_label = create_test_label(2);
        let child2_traversal = create_test_edge_traversal(2, 15.0);
        tree.insert(child2_label.clone(), child2_traversal.clone(), root_label.clone()).unwrap();
        ...
        assert_eq!(child1_node.edge_traversal.as_ref().unwrap().edge_id, EdgeId(1));
        
        let child2_node = tree.get(&child2_label).unwrap();
        assert!(!child2_node.is_root());
        assert_eq!(child2_node.parent, Some(root_label.clone()));
        assert_eq!(child2_node.edge_traversal.as_ref().unwrap().edge_id, EdgeId(2));
    }
    ...
    fn test_get_parent() {
        ...
        let child_label = create_test_label(1);
        let child_traversal = create_test_edge_traversal(1, 10.0);
        tree.insert(child_label.clone(), child_traversal, root_label.clone()).unwrap();
    ...
    fn test_reconstruct_path_forward_orientation() {
        ...
        let child2_label = create_test_label(2);
        let child2_traversal = create_test_edge_traversal(2, 15.0);
        tree.insert(child2_label.clone(), child2_traversal.clone(), child1_label.clone()).unwrap();
        
        let child3_label = create_test_label(3);
        let child3_traversal = create_test_edge_traversal(3, 20.0);
        tree.insert(child3_label.clone(), child3_traversal.clone(), child2_label.clone()).unwrap();
    ...
    fn test_iterators() {
        ...
        let child1_label = create_test_label(1);
        let child1_traversal = create_test_edge_traversal(1, 10.0);
        tree.insert(child1_label.clone(), child1_traversal, root_label.clone()).unwrap();
        
        let child2_label = create_test_label(2);
        let child2_traversal = create_test_edge_traversal(2, 15.0);
        tree.insert(child2_label.clone(), child2_traversal, root_label.clone()).unwrap();
    ...
    fn test_backtrack_with_direction_override() {
        ...
        let forward_path = tree.backtrack_with_direction(VertexId(3), Direction::Forward).unwrap();
        ...
        let reverse_path = tree.backtrack_with_direction(VertexId(3), Direction::Reverse).unwrap();
    ...
    fn test_backtrack_nonexistent_vertex() {
        ...
        assert!(matches!(result, Err(SearchTreeError::VertexNotFound(VertexId(99)))));
    }
    ...
    fn test_auto_root_creation() {
        ...
        tree.insert(child_label.clone(), edge_traversal.clone(), parent_label.clone()).unwrap();
        ...
        assert_eq!(child_node.edge_traversal.as_ref().unwrap().edge_id, EdgeId(1));
    }
    ...
    fn test_auto_root_creation_chain() {
        ...
        tree.insert(label1.clone(), create_test_edge_traversal(1, 10.0), label0.clone()).unwrap();
        
        // Subsequent inserts work normally
        tree.insert(label2.clone(), create_test_edge_traversal(2, 15.0), label1.clone()).unwrap();
        tree.insert(label3.clone(), create_test_edge_traversal(3, 20.0), label2.clone()).unwrap();
    ...
    fn test_insert_without_auto_root_when_parent_exists() {
        ...
        let child_label = create_test_label(1);
        let edge_traversal = create_test_edge_traversal(1, 10.0);
        tree.insert(child_label.clone(), edge_traversal, root_label.clone()).unwrap();
        ...
        let result = tree.insert(orphan_label, create_test_edge_traversal(99, 5.0), nonexistent_parent.clone());
        assert!(matches!(result, Err(SearchTreeError::ParentNotFound(_))));
    }
}
