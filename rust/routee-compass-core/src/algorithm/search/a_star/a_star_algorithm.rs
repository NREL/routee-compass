...
fn prune_tree(
    ...
    let prev_entries = tree.get_labels_iter(*label.vertex_id()) 
        .map(|label| {
            let node = tree.get(&label).ok_or_else(|| SearchTreeError::MissingNodeForLabel(label.clone()))?;
            let cost = node.traversal_cost().map(|tc| tc.objective_cost.clone());
