...
pub enum NetworkUtilityMapping {
    ...
    Combined(Vec<NetworkUtilityMapping>, CostAggregation),
}
...
impl NetworkUtilityMapping {
    pub fn traversal_cost(
        ...
        match self {
            ...
            NetworkUtilityMapping::Combined(mappings, op) => {
                let mapped = mappings
                    .iter()
    ...
    pub fn access_cost(
        ...
        match self {
            ...
            NetworkUtilityMapping::EdgeEdgeLookup { lookup } => match prev_edge {
                None => Ok(Cost::ZERO),
                Some(prev) => {
                    let result = lookup
                        .get(&(prev.edge_id, next_edge.edge_id))
                        .unwrap_or(&Cost::ZERO);
                    Ok(*result)
                }
            },
