...
use crate::{
    ...
    model::{filter::FilterModel, network::Edge},
};
        ...
        match self {
            ...
            MT::VertexId => {
                ...
                for edge in edges.into_iter() {
                    let fm =  si.get_filter_model(&edge.edge_list_id).map_err(|e| MapError::InternalError(format!("while map matching vertex_id {vertex_id}, failed to retrieve filter model for out edge list '{}', edge '{}': {e}", edge.edge_list_id, edge.edge_id)))?;
                    if let Ok(true) = test_edge(edge, fm) {
                        return Ok(());
                    }
                }
                Err(MapError::MapMatchError(format!("attempted to map match origin vertex_id {vertex_id} provided in query, but no out-edges are valid for traversal according to this FilterModel instance")))
            }
            MT::EdgeId => {
                // validate this edge
                let (edge_list_id, edge_id) = query.get_origin_edge()?;
                let fm =  si.get_filter_model(&edge_list_id).map_err(|e| MapError::InternalError(format!("while map matching edge_list_id '{edge_list_id}', edge_id '{edge_id}', failed to retrieve filter model for out edge list '{edge_list_id}', edge '{edge_id}': {e}")))?;
            ...
            MT::Point => {
                ...
                // are within our matching tolerance and validate them with the filter model
                ...
                for nearest in si.map_model.spatial_index.nearest_graph_id_iter(&src_point) {
                    match nearest {
                        NearestSearchResult::NearestVertex(vertex_id) => {
                            ...
                            let edges = si.graph.out_edges(&vertex_id).iter().map(|(edge_list_id, edge_id)| si.graph.get_edge(edge_list_id, edge_id)).collect::<Result<Vec<_>, _>>().map_err(|e| MapError::MapMatchError(format!("while attempting to validate vertex id {vertex_id} for map matching, the underlying Graph model caused an error: {e}")))?;
                            for edge in edges.into_iter() {
                                let fm =  si.get_filter_model(&edge.edge_list_id).map_err(|e| MapError::InternalError(format!("while map matching point {}, failed to retrieve filter model for out edge list '{}', edge '{}': {e}", src_point.to_wkt(), edge.edge_list_id, edge.edge_id)))?;
            ...
            MT::VertexId => {
                ...
                match vertex_id_option {
                    Some(vertex_id) => {
                        ...
                        for edge in edges.into_iter() {
                            let fm =  si.get_filter_model(&edge.edge_list_id).map_err(|e| MapError::InternalError(format!("while map matching destination vertex_id {vertex_id}, failed to retrieve filter model for out edge list '{}', edge '{}': {e}", edge.edge_list_id, edge.edge_id)))?;
                            if let Ok(true) = test_edge(edge, fm) {
                                return Ok(MapInputResult::Found);
                            }
                        }
                        Err(MapError::MapMatchError(format!("attempted to map match destination vertex_id {vertex_id} provided in query, but no in-edges are valid for traversal according to this FilterModel instance")))
                    }
                ...
                for nearest in si.map_model.spatial_index.nearest_graph_id_iter(&dst_point) {
                    match nearest {
                        NearestSearchResult::NearestVertex(vertex_id) => {
                            ...
                            let edges = si.graph.out_edges(&vertex_id).iter().map(|(edge_list_id, edge_id)| si.graph.get_edge(edge_list_id, edge_id)).collect::<Result<Vec<_>, _>>().map_err(|e| MapError::MapMatchError(format!("while attempting to validate vertex id {vertex_id} for map matching, the underlying Graph model caused an error: {e}")))?;
                            for edge in edges.into_iter() {
                                let fm =  si.get_filter_model(&edge.edge_list_id).map_err(|e| MapError::InternalError(format!("while map matching point '{}', failed to retrieve filter model for out edge list '{}', edge '{}': {e}", dst_point.to_wkt(), edge.edge_list_id, edge.edge_id)))?;
                                let is_valid = test_edge(edge, fm)?;
                                if is_valid {
...
fn test_edge(edge: &Edge, fm: Arc<dyn FilterModel>) -> Result<bool, MapError> {
    let is_valid = fm.valid_edge(edge).map_err(|e| MapError::MapMatchError(format!("while attempting to validate edge id {} for map matching, the underlying FilterModel caused an error: {}", edge.edge_id, e)))?;
    Ok(is_valid)
}

fn validate_edge(edge: &Edge, fm: Arc<dyn FilterModel>) -> Result<(), MapError> {
    let is_valid = test_edge(edge, fm)?;
    if !is_valid {
        Err(MapError::MapMatchError(format!(
            "query assigned origin of edge {} is not valid according to the FilterModel",
            edge.edge_id
        )))
    } else {
        Ok(())
