        ...
        match &self.turn_delay_model {
            TurnDelayModel::TabularDiscrete { table } => {
                ...
                let delay = table.get(&turn).ok_or_else(|| {
                    TraversalModelError::TraversalModelFailure(format!("table missing entry for turn {turn}"))
                })?;
                Ok(*delay)
            }
        }
...
pub fn get_headings(
    ...
    let heading: &EdgeHeading =
        headings_table
            ...
            .ok_or_else(|| TraversalModelError::TraversalModelFailure(format!("missing edge id {edge_id} ")))?;
    Ok(*heading)
}
