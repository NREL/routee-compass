pub fn network_from_edgelist_csv<S>(
    path: Path,
    edge_id_col: &str,
    ...
) -> Result<DirectedGraph, GraphError> {
    todo!()
}

pub fn from_edgelist_csv<S>(
    path: Path,
) -> Fn((Vertex, Edge, Vertex, S)) -> Result<Cost, TraversalError> {
    todo!()
}

pub fn from_edgeedgelist_csv<S>(
    path: Path,
) -> Fn((Vertex, Edge, Vertex, Edge, Vertex, S)) -> Result<Cost, TraversalError> {
    todo!()
}

pub fn from_zonelist_csv<S>(
    path: Path,
    v1_col_name: &str,
    ...
) -> Fn((Vertex, Edge, Vertex, Edge, Vertex, S)) -> Result<Cost, TraversalError> {
